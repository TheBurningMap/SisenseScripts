...
echo "Fetching dashboard OIDs that are on github"
...
foreach($dashFile in $dashFiles)
{
	...
	$dashOID = echo $dashData.oid | Out-File -FilePath $gitfolder\OIDs.txt -append
}
...
Get-Content OIDs.txt | foreach {
	...
	echo "$_"
		...
		Else
		{
			$responseDASH | ConvertTo-Json -Depth 15 | Set-Content "$responseTitle.dash"
		}
		cd $hostDir
		$CompleteFolderPath  = ""
		$responseTitle = ""
		...
		$responseTitle2 = ""
		$responseDASH= ""
	  } catch { "WARN: $OID OID doesn't exist on Sisense.." }
